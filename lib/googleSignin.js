...
angular.module('googleSignin', []).
  directive('googleSignin', ['$window', '$rootScope', function ($window, $rootScope) {
      ...
      return {
          ...
          link: function (scope, element, attrs, ctrl, linker) {
              ...
              function onSignIn(authResult) {
                  $rootScope.$broadcast('event:google-signin-success') //, authResult);
              };
              function onSignInFailure() {
                  $rootScope.$broadcast('event:google-signin-failure') //, null);
              };

              function setSigninState(isSignedIn) {
                if (isSignedIn) {
                  // var authInst = gapi.auth2.getAuthInstance()
                  // var authResp = authInst.getAuthResponse()
                  $rootScope.$broadcast('event:google-signin-success') //, authResult)
                } else {
                  $rootScope.$broadcast('event:google-signin-failure') //, null)
                }
                  ...
                  po.onload = function () {
                      ...
                      gapi.load('client:auth2', function () {
                          ...
                          gapi.client.init({
                            ...
                            console.log("gapi init callback")
                            $rootScope.$broadcast('event:gapi-initialized')
                            gapi.auth2.getAuthInstance().isSignedIn.listen(setSigninState)
                            setSigninState(gapi.auth2.getAuthInstance().isSignedIn.get())
                          })
